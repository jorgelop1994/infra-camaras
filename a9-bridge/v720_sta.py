...
class v720_sta(log):
    ...
    def __on_mjpg_rcv_hnd(self, conn: netsrv_udp, payload: bytes):
        ...
        if pkg.msg_flag == cmd_udp.PROTOCOL_MSG_FLAG_HEAD \
            ...
            # Assemble complete frame from all fragments FIRST
            frame = bytearray()
            while not self._vframe.empty():
                frame.extend(self._vframe.get(False))

            # Validate: skip corrupted frames (missing JPEG EOI = fragment loss)
            if len(frame) > 2 and frame[-2:] == b'\xff\xd9':
                # Frame is complete - distribute to all callbacks
                with self._cb_mtx:
                    for cb in self._vframe_cb:
                        cb(self, bytes(frame))
            else:
                self.dbg(f'Drop corrupted frame (missing EOI), sz={len(frame)}')
