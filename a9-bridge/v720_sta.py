...
class v720_sta(log):
    ...
    def __init__(self, tcp_conn: netsrv_tcp, udp_conn: netsrv_udp = None, videoframe_cb: function = None, audioframe_cb: function = None, init_done_cb: function = None, disconnect_cb: function = None) -> None:
        ...
        self._frame_lst = []
        self._frame_pkg_start = None  # pkg_id of HEAD fragment for gap detection
        self._frame_frag_count = 0     # fragments received for current frame
    ...
    def __fwd_resp_hnd(self, conn: netsrv_tcp, pkg: prot_json_udp):
    ...
    def __on_mjpg_rcv_hnd(self, conn: netsrv_udp, payload: bytes):
        ...
        # SINGLE COMPLETE FRAME (no fragmentation)
        if pkg.msg_flag == cmd_udp.PROTOCOL_MSG_FLAG_FINISH:
            frame = pkg.payload
            if len(frame) > 2 and frame[-2:] == b"\xff\xd9":
                ...
                self.dbg(f"Drop corrupted FINISH frame (missing EOI), sz={len(frame)}")
            return

        # HEAD = start of a new fragmented frame
        if pkg.msg_flag == cmd_udp.PROTOCOL_MSG_FLAG_HEAD:
            self._vframe = Queue()
            self._frame_pkg_start = pkg._pkg_id
            self._frame_frag_count = 0

        # Collect fragment data
        if pkg.msg_flag in (cmd_udp.PROTOCOL_MSG_FLAG_HEAD, cmd_udp.PROTOCOL_MSG_FLAG_BODY):
            self._vframe.put(pkg.payload)
            self._frame_frag_count += 1
            ...
            self.dbg(f"Receive MJPEG frame sz: {sz}, fragments: {self._frame_frag_count}")

            # Assemble all fragments for this frame
            frame = bytearray()
            while not self._vframe.empty():
                frame.extend(self._vframe.get(False))

            # Verify: all expected fragments arrived + valid JPEG EOI
            expected = pkg._pkg_id - self._frame_pkg_start + 1 if self._frame_pkg_start is not None else 0
            all_frags = expected > 0 and self._frame_frag_count >= expected
            eoi_ok = len(frame) > 2 and frame[-2:] == b"\xff\xd9"

            if all_frags and eoi_ok:
                with self._cb_mtx:
                    for cb in self._vframe_cb:
                        cb(self, bytes(frame))
            elif not all_frags:
                self.dbg(f"Drop frame (missing fragment, {self._frame_frag_count}/{expected}), sz={len(frame)}")
            else:
                self.dbg(f"Drop frame (missing EOI), sz={len(frame)}")
